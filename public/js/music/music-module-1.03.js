...
function musicModule() {
    ...
    this.openStore = function () {
        ...
        let store = indexedDB.open("musicstore", 14);
        ...
        store.onupgradeneeded = function (event) {
            ...
            if (!db.objectStoreNames.contains('currentSong')) {
                db.createObjectStore('currentSong', {keyPath: "idx"});
            }
        }
        ...
        $('#SongList').dxDataGrid({
            ...
            dataSource: new DevExpress.data.DataSource({
                load: function (loadOptions) {
                    ...
                    loadOptions.criteria = MusicApplication.criteria;
                    $.ajax({
    ...
    this.saveSelectedItem = function () {
        ...
        let item = {idx: 0, id: index};
        let transaction = _self.db.transaction("currentSong", "readwrite");

        let request = transaction.objectStore("currentSong").put(item);
        request.onerror = function (e) {
            console.error(e);
        }
    ...
    this.getSelectedItem = function () {
        let transaction = _self.db.transaction(['currentSong'], 'readonly');
        let store = transaction.objectStore('currentSong');
    ...
    this.clearSelectedItem = function () {
        ...
        let request = transaction.objectStore("currentSong").clear();
